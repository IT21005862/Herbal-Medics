...
import SellerProductUI from "./AllsellersProductUI";
import SearchIcon from "@mui/icons-material/Search";
import InputAdornment from '@mui/material/InputAdornment';
import TextField from '@mui/material/TextField';
//IT21013300

// Product of seller function
const Productperseller = () => {   //This function displays all products
  const userid = useSelector((state) => state.user._id);
  const [product, setProducts] = useState([]);

  const fetchDetails = async () => {
    const res = await axios
      .get(`http://localhost:9001/product/getallProducts`)
  ...
  const search = async () => {
    const res = await axios
      .get(`http://localhost:9001/product/getallProducts`) //Get all the products from the DB
      .catch((err) => console.log(err));
    const data = await res.data;
    return data;
  };
